...
i=0;
h=0;
mapfile -t ids < <(jq -r .result[].packageId "${WORKSPACE}/out.json")
...
mapfile -t cdates < <(jq -r .result[].createdDate "${WORKSPACE}/out.json")
mapfile -t cbys < <(jq -r .result[].createdBy "${WORKSPACE}/out.json")
...
do 
    ...
    printf  "%s\n%s\n%s\n%s\n%s\n%s\n%s\n\n"  "<tr><th>${h}</th>" "<th>${name}</th>" "<th>${pvs[$i]}</th>" "<th>${ctypes[$i]}</th>" "<th>${cdates[$i]}</th>" "<th>${cbys[$i]}</th>" "<th>${notes[$i]}</th></tr>";	
		i=$(( $i + 1 )); 
done
...
do
	...
	mapfile -t pvs < <(jq -r .result[].packageVersion "${WORKSPACE}/out.json")
	mapfile -t ctypes < <(jq -r .result[].componentType "${WORKSPACE}/out.json")
	mapfile -t cdates < <(jq -r .result[].deployedDate "${WORKSPACE}/out.json")
	mapfile -t cbys < <(jq -r .result[].deployedBy "${WORKSPACE}/out.json")
	mapfile -t notes < <(jq -r .result[].notes "${WORKSPACE}/out.json")
